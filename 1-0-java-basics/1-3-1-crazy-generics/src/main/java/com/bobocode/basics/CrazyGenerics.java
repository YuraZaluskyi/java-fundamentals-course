...
/**
 * {@link CrazyGenerics} is an exercise class. It consists of classes, interfaces and methods that
 * should be updated using generics.
 * <p>
...
public class CrazyGenerics {
  ...
  /**
   * {@link Sourced} is a container class that allows storing any object along with the source of
   * that data. The value type can be specified by a type parameter "T".
   *
   * @param <T> – value type
   */
  @Data
  ...
  /**
   * {@link Limited} is a container class that allows storing an actual value along with possible
   * min and max values. It is special form of triple. All three values have a generic type that
   * should be a subclass of {@link Number}.
   *
   ...
   * independent generic types. It defines a convert method which accepts one parameter of one type
   * and returns a converted result of another type.
   *
  ...
  /**
   * {@link MaxHolder} is a container class that keeps track of the maximum value only. It works
   * with comparable objects and allows you to put new values. Every time you put a value, it is
   * stored only if the new value is greater than the current max.
   *
   * @param <T> – value type
   */
  public static class MaxHolder<T extends Comparable<T>> { // todo: refactor class to make it generic
    ...
    /**
     * Puts a new value to the holder. A new value is stored to the max, only if it is greater than
     * current max value.
     *
...
//      throw new ExerciseNotCompletedException(); // todo: update parameter and implement the method
  ...
  /**
   * {@link StrictProcessor} defines a contact of a processor that can process only objects that are
   * {@link Serializable} and {@link Comparable}.
   *
  ...
  /**
   * {@link CollectionRepository} defines a contract of a runtime store for entities based on any
   * {@link Collection}. It has methods that allow to save new entity, and get whole collection.
   *
   ...
   * collection as {@link java.util.List}.
   *
   * @param <T> – a type of the entity that should be a subclass of {@link BaseEntity}
   */
  ...
  /**
   * {@link ComparableCollection} is a {@link Collection} that can be compared by size. It extends a
   * {@link Collection} interface and {@link Comparable} interface, and provides a default
   * implementation of a compareTo method that compares collections sizes.
   * <p>
   * Please note that size does not depend on the elements type, so it is allowed to compare
   * collections of different element types.
   *
  ...
  static class CollectionUtil {
    ...
    static final Comparator<BaseEntity> CREATED_ON_COMPARATOR = Comparator.comparing(
        BaseEntity::getCreatedOn);
    ...
    /**
     * Util method that check if provided collection has new entities. An entity is any object that
     * extends {@link BaseEntity}. A new entity is an entity that does not have an id assigned. (In
     * other word, which id value equals null).
     *
    ...
    /**
     * Util method that checks if a provided collection of entities is valid. An entity is any
     * subclass of a {@link BaseEntity} A validation criteria can be different for different cases,
     * so it is passed as second parameter.
     *
     * @param entities provided collection of entities
    ...
    /**
     * hasDuplicates is a generic util method checks if a list of entities contains target entity
     * more than once. In other words, it checks if target entity has duplicates in the provided
     * list. A duplicate is an entity that has the same UUID.
     *
     ...
     * element with the index i will be located on index j, and the element with index j, will be
     * located on the index i. Please note that in order to make it convenient and simple, it DOES
     * NOT declare any type parameter.
     *
